#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: Ranged3PCamera

#include "Basic.hpp"

#include "FortniteGame_classes.hpp"


#pragma pack(push, 0x4)
namespace SDK
{

// BlueprintGeneratedClass Ranged3PCamera.Ranged3PCamera_C
// 0x0000 (0x0110 - 0x0110)
class URanged3PCamera_C final : public UFort3PCam_Default
{
public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"Ranged3PCamera_C">();
	}
	static class URanged3PCamera_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<URanged3PCamera_C>();
	}
};
static_assert(alignof(URanged3PCamera_C) == 0x000004, "Wrong alignment on URanged3PCamera_C");
static_assert(sizeof(URanged3PCamera_C) == 0x000110, "Wrong size on URanged3PCamera_C");

}

#pragma pack(pop)
