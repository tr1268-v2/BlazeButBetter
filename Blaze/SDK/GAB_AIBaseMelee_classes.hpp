#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: GAB_AIBaseMelee

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "FortniteGame_classes.hpp"


#pragma pack(push, 0x4)
namespace SDK
{

// GameplayAbilityBlueprintGeneratedClass GAB_AIBaseMelee.GAB_AIBaseMelee_C
// 0x000C (0x06E8 - 0x06DC)
class UGAB_AIBaseMelee_C : public UFortGameplayAbility
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame;                                    // 0x06DC(0x0004)(ZeroConstructor, Transient, DuplicateTransient)
	bool                                          Debug;                                             // 0x06E0(0x0001)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, IsPlainOldData, NoDestructor)
	bool                                          UseMoveForwardAction;                              // 0x06E1(0x0001)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          MoveGate;                                          // 0x06E2(0x0001)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          UseRotationRateModification;                       // 0x06E3(0x0001)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor)
	float                                         MeleeRotationRate;                                 // 0x06E4(0x0004)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor)

public:
	void ExecuteUbergraph_GAB_AIBaseMelee(int32 EntryPoint);
	void K2_ActivateAbility();
	void Triggered_8CCC8A6B41B7B728038BDE887BD23BCC(const struct FGameplayAbilityTargetDataHandle& TargetData, const struct FGameplayTag& ApplicationTag);
	void Cancelled_8CCC8A6B41B7B728038BDE887BD23BCC(const struct FGameplayAbilityTargetDataHandle& TargetData, const struct FGameplayTag& ApplicationTag);
	void Completed_8CCC8A6B41B7B728038BDE887BD23BCC(const struct FGameplayAbilityTargetDataHandle& TargetData, const struct FGameplayTag& ApplicationTag);

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"GAB_AIBaseMelee_C">();
	}
	static class UGAB_AIBaseMelee_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UGAB_AIBaseMelee_C>();
	}
};
static_assert(alignof(UGAB_AIBaseMelee_C) == 0x000004, "Wrong alignment on UGAB_AIBaseMelee_C");
static_assert(sizeof(UGAB_AIBaseMelee_C) == 0x0006E8, "Wrong size on UGAB_AIBaseMelee_C");
static_assert(offsetof(UGAB_AIBaseMelee_C, UberGraphFrame) == 0x0006DC, "Member 'UGAB_AIBaseMelee_C::UberGraphFrame' has a wrong offset!");
static_assert(offsetof(UGAB_AIBaseMelee_C, Debug) == 0x0006E0, "Member 'UGAB_AIBaseMelee_C::Debug' has a wrong offset!");
static_assert(offsetof(UGAB_AIBaseMelee_C, UseMoveForwardAction) == 0x0006E1, "Member 'UGAB_AIBaseMelee_C::UseMoveForwardAction' has a wrong offset!");
static_assert(offsetof(UGAB_AIBaseMelee_C, MoveGate) == 0x0006E2, "Member 'UGAB_AIBaseMelee_C::MoveGate' has a wrong offset!");
static_assert(offsetof(UGAB_AIBaseMelee_C, UseRotationRateModification) == 0x0006E3, "Member 'UGAB_AIBaseMelee_C::UseRotationRateModification' has a wrong offset!");
static_assert(offsetof(UGAB_AIBaseMelee_C, MeleeRotationRate) == 0x0006E4, "Member 'UGAB_AIBaseMelee_C::MeleeRotationRate' has a wrong offset!");

}

#pragma pack(pop)
